...
The local runner executes once per task and detects elapsed-time overruns after return. Airflow provides actual scheduling, retry delays and task timeout enforcement. CPU/memory declarations are metadata; worker resource isolation must be configured in Airflow. One active DAG run is configured, but separate DAGs writing the same destination require administrative coordination. Cross-file fan-out is not an atomic distributed transaction.
